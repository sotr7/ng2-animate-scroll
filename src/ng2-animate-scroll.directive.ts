...
export class AnimateScrollDirective implements AfterViewInit {
    ...
    @Input('animatescroll') options: Options;
    @Output('animatescroll') output = new EventEmitter();
    @Output('animatescrollAll') outputStats = new EventEmitter();
    classNameSetByAnimateScroll: string = '';
    ...
    private _processEvent() {
        if (!this.options || typeof (this.options) === 'string' || typeof (this.options) === 'number') {
            ...
            console.warn('AnimateScroll:', 'Invalid options provided:', this.options);
        }
        else {
    ...
    private _applyDefaultStates() {
        let stateApplied: boolean = false;
        let defaultStates = this.animateScrollService.getDefaultStates();
        ...
        if (!stateApplied) {
            ...
            console.warn('AnimateScroll:', 'No state matched current scroll position, please provide a default state!');
        }
    }
    private _processState(state: State): boolean {
        ...
        if (this._isValidState(state)) {
            ...
            switch (state.method.toLowerCase()) {
                ...
                default:
                    console.warn('AnimateScroll:', 'Provided method doesn\'t match any possible methods: ', state.method);
                    return false;
            }
        }
        return false;
    ...
    private _isValidState(state: State): boolean {
        if (typeof (state.method) === 'undefined') {
            console.warn('AnimateScroll:', 'No method was provided for a state but is required.');
            return false;
        }
        if (typeof (state.method) !== 'string') {
            console.warn('AnimateScroll:', 'Invalid value for method:string provided : ', state.method);
            return false;
        }
        if (state.method !== 'default' && typeof (state.value) === 'undefined') {
            console.warn('AnimateScroll:', 'No value was provided for a state but is required if method isn\'t default.');
            return false;
        }
        if (state.method !== 'default' && typeof (state.value) !== 'number') {
            console.warn('AnimateScroll:', 'Invalid value for value:number provided : ', state.value);
            return false;
        }
        if (typeof (state.state) !== 'undefined' && typeof (state.state) !== 'string') {
            console.warn('AnimateScroll:', 'Invalid value for state:string provided : ', state.state);
            return false;
        }
        if (typeof (state.sizes) !== 'undefined' && typeof (state.sizes) !== 'string') {
            console.warn('AnimateScroll:', 'Invalid value for sizes:string provided : ', state.sizes);
            return false;
        }
        if (typeof (state.class) !== 'undefined' && typeof (state.class) !== 'string') {
            console.warn('AnimateScroll:', 'Invalid value for class:string provided : ', state.class);
            return false;
        }
        if (typeof (state.setAtLastChance) !== 'undefined' && typeof (state.setAtLastChance) !== 'boolean') {
            console.warn('AnimateScroll:', 'Invalid value setAtLastChance:boolean provided : ', state.sizes);
            return false;
        }
        return true;
    }
    ...
    private _setClass(stateClass: string) {
        if (typeof (stateClass) !== 'undefined' && this.classNameSetByAnimateScroll !== stateClass) {
            //class is provided and has to be applied to the element
            if (this.classNameSetByAnimateScroll !== '') {
                this.el.nativeElement.className = this.el.nativeElement.className.replace(this.classNameSetByAnimateScroll, stateClass);
            }
            else {
                this.renderer.setElementClass(this.el.nativeElement, stateClass, true);
            }
            this.classNameSetByAnimateScroll = stateClass;
        }
        else {
            //no class set for current state, remove class if one is set
            if (this.classNameSetByAnimateScroll !== '') {
                this.el.nativeElement.className = this.el.nativeElement.className.replace(this.classNameSetByAnimateScroll, '');
            }
            this.classNameSetByAnimateScroll = '';
        }
    }
    ...
    private _isCurrentScreenSize(screenSizes: string): boolean {
        ...
        if (positionInPossibleScreenSizesArray === -1) {
            ...
            console.warn('AnimateScroll:', 'Invalid screen sizes provided:', screenSizes);
            return true;
        }
        if (direction === 'up') {
